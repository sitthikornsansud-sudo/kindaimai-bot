...
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY", "")  # จาก aistudio.google.com ฟรี ไม่ผูกบัตร อ่านไทยเก่ง
GEMINI_MODEL = os.environ.get("GEMINI_MODEL", "gemini-2.5-flash")
OCRSPACE_API_KEY = os.environ.get("OCRSPACE_API_KEY", "")  # สำรองสุดท้าย (อังกฤษเท่านั้น)
...
# ══════════════ OCR: Google Vision (หลัก) + OCR.space (สำรอง ฟรีไม่ผูกบัตร) ══════════════
def ocr_google(image_bytes: bytes) -> str:
    ...
    if "error" in data.get("responses", [{}])[0]:
        raise RuntimeError(data["responses"][0]["error"].get("message", "vision error"))
    try:
...
def shrink_image(image_bytes: bytes, max_side=1600, quality=85) -> bytes:
    """ย่อรูปให้เล็กกว่า ~900KB (OCR.space ฟรีรับไม่เกิน 1MB) โดยคงความคมของตัวหนังสือ"""
    try:
        ...
        while True:
            ...
            data = buf.getvalue()
            if len(data) <= 900_000 or (max_side <= 800 and quality <= 60):
                print(f"shrink_image: {len(image_bytes)} -> {len(data)} bytes")
                return data
            max_side = int(max_side * 0.8)
            quality = max(55, quality - 10)
    except Exception as e:
        print("shrink_image ล้มเหลว ใช้รูปเดิม:", e)
        return image_bytes


def ocr_gemini(image_bytes: bytes) -> str:
    """อ่านฉลากด้วย Gemini (Google AI Studio) — ฟรี ไม่ผูกบัตร รองรับภาษาไทยดีมาก"""
    small = shrink_image(image_bytes)
    ...
    payload = {
        "contents": [{
            "parts": [
                ...
                {"text": "ถอดข้อความทั้งหมดที่อยู่ในภาพนี้ออกมาแบบคำต่อคำ ทุกบรรทัด "
                          "รวมชื่อสินค้า ส่วนประกอบ และวันหมดอายุ (EXP/BBF) "
                          "ตอบเป็นข้อความล้วนเท่านั้น ห้ามอธิบายเพิ่ม"},
            ]
        }],
    ...
    try:
        ...
        print("Gemini ตอบรูปแบบไม่คาดคิด:", str(data)[:300])
        return ""


def _ocr_space_call(b64: str, language: str, engine: int) -> str:
    r = requests.post(
    ...
    if data.get("IsErroredOnProcessing"):
        print("OCR.space แจ้ง error:", data.get("ErrorMessage"))
        return ""
...
def ocr_space(image_bytes: bytes) -> str:
    """สำรองสุดท้าย: OCR.space อ่านได้เฉพาะภาษาอังกฤษ (ไม่รองรับไทย)"""
    small = shrink_image(image_bytes)
    b64 = base64.b64encode(small).decode()
    return _ocr_space_call(b64, "eng", 2)


def ocr_image(image_bytes: bytes) -> str:
    """ลำดับ: Google Vision → Gemini (ตัวหลักถ้าไม่มี Vision) → OCR.space (อังกฤษ)"""
    if GOOGLE_VISION_API_KEY:
        try:
            text = ocr_google(image_bytes)
            if text.strip():
                return text
        except Exception as e:
            print("Google Vision ใช้ไม่ได้ ไปตัวถัดไป:", e)
    if GEMINI_API_KEY:
        try:
            text = ocr_gemini(image_bytes)
            if text.strip():
                return text
        except Exception as e:
            print("Gemini ใช้ไม่ได้ ไปตัวถัดไป:", e)
    if OCRSPACE_API_KEY:
        try:
            return ocr_space(image_bytes)
        except Exception as e:
            print("OCR.space ล้มเหลว:", e)
    return ""
